...
class OllamaBackend:
    ...
    def send(self, messages, tools, system_prompt):
        ...
        # Note: tool-calling support varies by local model; many coder models
        # (e.g. qwen2.5-coder) support it via Ollama's /api/chat tools field.
        if tools:
            payload["tools"] = [{"type": "function", "function": t} for t in tools]
        resp = requests.post(self.cfg["base_url"], json=payload, timeout=300)
        if resp.status_code != 200:
